...
public class AlertHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		System.out.println("AAAAAA"+senderId);
		...
		if (StringUtils.hasText(msg)) {  //메세지 내용이 있으면 
			...
			if(strs != null && strs.length == 6) {
				...
				System.out.println("boardWriterSession"+boardWriterSession);
					...
					//내 지꾸 글에 댓글달림 && 글쓴이가 로그인해있음
					if ("jcomment".equals(cmd) && boardWriterSession != null) {
						
						System.out.println("onmessage되나?");
						
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 지꾸 " + title+"에 댓글을 달았습니다!");
						boardWriterSession.sendMessage(tmpMsg); //글작성자에게 메세지 보냄 
					}
					
					//내 모꾸 글에 댓글달림 && 글쓴이가 로그인해있음
					else if ("mcomment".equals(cmd) && boardWriterSession != null) {
						
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 모꾸 " + title+"에 댓글을 달았습니다!");
						boardWriterSession.sendMessage(tmpMsg); //글작성자에게 메세지 보냄 
					}
					
	
					//내 지꾸 댓글에 대댓글달림 && 글쓴이가 로그인해있음 
					else if ("jcocomment".equals(cmd) && comtWriter != null) {
					...
					//내 모꾸 댓글에 대댓글달림 && 글쓴이가 로그인해있음 
					else if ("mcocomment".equals(cmd) && comtWriter != null) {
						//comtWriter = 대댓단사람 , boardWriter = 댓글단사람
					...
					//내 지꾸 글에 좋아요 && 글쓴이가 로그인해있음
					else if("jlike".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 좋아요누른사람 , boardWriter = 게시글작성자
						
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 지꾸 " + title+ " 에 작성한 글을 좋아요했습니다!</a>");
						boardWriterSession.sendMessage(tmpMsg);
					}
					
					//내 모꾸 글에 좋아요 && 글쓴이가 로그인해있음
					else if("mlike".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 좋아요누른사람 , boardWriter = 게시글작성자
						
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 모꾸 " + title+ " 에 작성한 글을 좋아요했습니다!</a>");
						boardWriterSession.sendMessage(tmpMsg);
					}
					
					//팔로우  && 상대방이 로그인해있음
					else if("follow".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 팔로우한사람 , boardWriter = 팔로우받은사람
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 " + boardWriter + "님을 팔로우 했습니다.");
						boardWriterSession.sendMessage(tmpMsg);
					}
					
					//채팅왔을때
					else if("chat".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 채팅건사람 , boardWriter = 채팅받은사람
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 " + boardWriter + "님께 채팅을 보냈습니다.");
						boardWriterSession.sendMessage(tmpMsg);
					}
				
					//모임 요청 승인시  
					else if("moqoook".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 모임주최자 , boardWriter = 모임신청한사람
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 " + boardWriter + "님의 모임신청을 승인했습니다.");
						boardWriterSession.sendMessage(tmpMsg);
					}
					...
					//모임 요청 거절시  
						...
						//comtWriter = 모임주최자 , boardWriter = 모임신청한사람
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 " + boardWriter + "님의 모임신청을 거절했습니다.");
						boardWriterSession.sendMessage(tmpMsg);
					}
				
					//모임 요청 받았을 시 
					else if("moqooreque".equals(cmd) && boardWriterSession != null) {
						//comtWriter = 모임신청자 , boardWriter = 모임주최자
						TextMessage tmpMsg = new TextMessage
								(comtWriter + "님이 " + boardWriter + "님께 모임을 신청했습니다.");
						boardWriterSession.sendMessage(tmpMsg);
					}
			}
...
//		Member loginUser = (Member)httpSession.get("userId");
